%dw 2.0
fun getLayer(errorType) = if (errorType.Layer?) errorType.Layer else Mule::p("app.layer")
...
fun customizedError(errorObject, httpError, levelValue) = 
	if ( errorObject is Null ) {
		...
		"customError": {
			...
			"type": "CORE-F-040",
			"location": levelValue  as String ++ " " ++ getLayer(errorObject.errorType),
	...
	else 
	{
		"errorType": (errorObject.errorType ++ {
			"Layer": getLayer(errorObject."errorType")})
		,
